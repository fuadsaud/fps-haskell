...
integrate (PowSe xs) = let scaleRecip (x:xs) n = (x/n):(scaleRecip xs (n+1))
                         in PowSe (scaleRecip xs 1)
...
-- sinPowSeD = shift (integrate ())

cosPowSeR :: PowSe Rational
cosPowSeR = let (PowSe xs) = integrate sinPowSeR
                in PowSe ((1%1):xs)
